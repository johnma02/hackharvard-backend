...
# https://www.kaggle.com/code/jonathanma02/hack-harvard-crime-model/edit
...
#https://www.kaggle.com/jonathanma02/hack-harvard-crash-model
@app.route('/predict_crash', methods=['POST'])
def predict_crash():
    """Endpoint returning a classification and regression value for crash prediction based on location and time data.
    ---
    parameters:
      - in: body
        name: input_data
    ...
    responses:
      200:
        description: Classification and regression values for crash prediction based on location and time data.
        schema:
          type: object
          properties:
            prediction:
    ...
    lng = data.get('lng')
    date = datetime.fromisoformat(data.get('date'))
    with open('./models/crash_model_OSVM.pkl', 'rb') as model_file:
        crash_model = pickle.load(model_file)
    prediction = None
